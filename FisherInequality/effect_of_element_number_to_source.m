...
for eleIndex = 1 : length(elementSize)
    figure(eleIndex)
    actualVariance =0;
    ...
    %     signalAngle=-1*randi([-90,90],1,2);%,-45,15,50]; % the angle of the signal
        ...
        snapshotNumber = 10;
        antennaNumber = elementSize(eleIndex);
...
%         loc =[3*rand(1,3), 3*rand(1,3)+deltaDis, 3*rand(1,3)+2*deltaDis,3*rand(1,3)+3*deltaDis]; 
    ...
    %     h.CapSize = 10;
        ...
        actualVariance = actualVariance + sum((actualWeight-recWeightsAvg).^2)/testNumber;
    end
