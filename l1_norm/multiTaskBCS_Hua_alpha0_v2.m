function [ time,weights,selectedIndex, recSigmaSquare,recoveredValuesSD] = ...
    multiTaskBCS_Hua_alpha0_v2( phi, y, eta )

% the delta marginal likelihood is modified
...
%     denominatorSum = 0;
%     for noiseIdx = 1 : L
%         numeratorSum = numeratorSum + sum((y{noiseIdx} - phiSelected{noiseIdx}*mu{noiseIdx}).^2);
%         denominatorSum = denominatorSum + (Row(noiseIdx) - length(selectedIndex) + alpha'*diag(covMatrix{noiseIdx}));
%     end
