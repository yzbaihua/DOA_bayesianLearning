function [time,weights,used,sigmaSquare,errbars,basis,selected,alpha,lambda] = ...
    MultiTaskFastLaplace(phi,y,sigmaSquare,eta,lambda_init, adaptive,optimal,scale, verbose)
...
% phi = phi(:,index);
% Hessian = alpha + phi'*phi/sigmaSquare;
% Sig = 1/Hessian;
% mu = Sig*PHIy(index)/sigmaSquare;
% left = phi'*phi/sigmaSquare;
% S = PHI2/sigmaSquare-Sig*left.^2;
% Q = PHIy/sigmaSquare-Sig*PHIy(index)/sigmaSquare*left;
...
%     is0 = find(theta < L*lambda);
    ...
    [ML(count),maxDeltaMLIdx] = max(sum(real(ml),2));
    ...
    if ismember(maxDeltaMLIdx, ire) % reestimate a basis
    ...
    elseif ismember(maxDeltaMLIdx, iad)
    ...
    elseif ismember(maxDeltaMLIdx,ide)
