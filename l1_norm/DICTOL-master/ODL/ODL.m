function [D, X] = ODL(Y, k, beta, opts, method)
% * Solving the following problem:
%  (D, X) = \arg\min_{D,X} 0.5||Y - DX||_F^2 + beta||X||_1
% * Syntax: `[D, X] = ODL(Y, k, beta, opts, sc_method)`
%   - INPUT: 
%     + `Y`: collection of samples.4/7/2016 7:35:39 PM
%     + `k`: number of atoms in the desired dictionary.
%     + `beta`: norm 1 regularization parameter.
%     + `opts`: option.
...
%     D = rand(size(Y,1),k);
    ...
    if opts.verbose 
        fprintf('cost: %f', ODL_cost(Y, D, X, beta));
    end 
    optsX = opts;
	optsX.max_iter = 200;
	optsX.tol      = 1e-8;
	...
	while iter < opts.max_iter
		iter = iter + 1;
		%% ========= sparse coding step ==============================
		X = lasso_fista(Y, D, X, beta, optsX);
       	if opts.verbose 
			costX = ODL_cost(Y, D, X, beta);
